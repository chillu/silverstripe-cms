...
class FilesystemPublisher extends StaticPublisher {
	...
	function publishPages($urls) { 
		...
		foreach($urls as $url => $path) {
			...
			if($url == "") $url = "/";
			// Decode the urlencoded URLSegments individually, which emulates the behaviour of
			// HTTP servers: It accepts encoded paths without multibyte characters, and converts it to UTF-8 on the fly.
			$url = implode('/', array_map('rawurldecode', explode('/', $url)));
			if(Director::is_relative_url($url)) $url = Director::absoluteURL($url);
			// URLSegment is stored
